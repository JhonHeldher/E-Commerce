...
const TopBar = () => {
    ...
    return (
        <div className="sticky top-0 z-20 w-full flex justify-between items-center px-8 py-4 bg-blue-20 shadow-xl lg:hidden">
            <Image
                src="/next.svg"
                alt="logo"
                width={150}
            ...
            <div className="flex gap-8 max-md:hidden">
                {navLinks.map((link) => (
                    <Link
                        href={link.url}
                        key={link.label}
                        className={`flex gap-4 text-boby-medium ${pathname === link.url ? "text-blue-500" : "text-gray-500"}`}
                    >
                        <p>{link.label}</p>
                    </Link>
                ))}
